...
import tk.maizbagwala.social.adapter.StoriesListAdapter;
import tk.maizbagwala.social.adapter.UserListAdapter;
import tk.maizbagwala.social.api.CommonClassForAPI;
import tk.maizbagwala.social.databinding.ActivityInstagramBinding;
import tk.maizbagwala.social.interfaces.UserListInterface;
import tk.maizbagwala.social.model.Edge;
import tk.maizbagwala.social.model.EdgeSidecarToChildren;
...
import tk.maizbagwala.social.model.story.TrayModel;
import tk.maizbagwala.social.util.AppLangSessionManager;
import tk.maizbagwala.social.util.SharePrefs;
...
import static tk.maizbagwala.social.util.Utils.RootDirectoryInsta;
import static tk.maizbagwala.social.util.Utils.createFileFolder;
import static tk.maizbagwala.social.util.Utils.startDownload;
...
//        InterstitialAdsINIT();
    ...
    private void initViews() {
        ...
        binding.rlBackInstagram.setOnClickListener(new View.OnClickListener() {
            @Override
...
//                showInterstitial();
    ...
    private void callDownload(String Url) {
        ...
        if (UrlWithoutQP.contains("reel")) {
            commonClassForAPI.getReelData(reelObserver, UrlWithoutQP);
        } else {
            try {
    ...
    private DisposableObserver<ResponseModel> reelObserver = new DisposableObserver<ResponseModel>() {
        @Override
        public void onNext(@NonNull ResponseModel reelModel) {
            Utils.hideProgressDialog(activity);
            if (reelModel.getGraphql() != null) {
                String VideoUrl = reelModel.getGraphql().getShortcode_media().getVideo_url();
                Utils.startDownload(VideoUrl, Utils.RootDirectoryInsta, activity, getVideoFilenameFromURL(VideoUrl));
            } else {
                Utils.setToast(activity, "Download from Private Account Please Login");
            }
        ...
        public void onError(@NonNull Throwable e) {
            Utils.hideProgressDialog(activity);
            e.printStackTrace();
            Utils.setToast(activity, "Try after login or something went wrong");
        }
                ...
                if (edgeSidecarToChildren != null) {
                    ...
                    for (int i = 0; i < edgeArrayList.size(); i++) {
                        if (edgeArrayList.get(i).getNode().isIs_video()) {
                            ...
                            Utils.startDownload(PhotoUrl, Utils.RootDirectoryInsta, activity, getImageFilenameFromURL(PhotoUrl));
                            PhotoUrl = "";
                            binding.etText.setText("");
                        }
                    }
